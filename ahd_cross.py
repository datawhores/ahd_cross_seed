...
def download(arguments,txt):
    ...
    for line in list:
        ...
        if os.path.isdir(line)==True:
            download_folder(arguments,txt,line,source)
        else:
            print("false")
            download_file(arguments,txt,line,source)
...
def scan(arguments,txt):
    ...
    for line in list:
        ...
        if line=='\n':
            continue
        if os.path.isdir(line)==True:
            scan_folder(arguments,txt,line,source)
        else:
            scan_file(arguments,txt,line,source)
        print("Waiting 5 Seconds")
        time.sleep(5)
"""
Scanning Functions
...
def searchnormal(arguments,ignorefile):
    ...
    for root in list:
        if os.path.isdir(root)==False:
          ...
          continue
        os.chdir(root)
        temp=subprocess.run([arguments['--fd'],'.',root,'--max-depth','1','--ignore-file',ignorefile],stdout=folders)
    print("Done")
    os.chdir(workingdir)
